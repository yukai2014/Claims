...
AdaptiveEndPoint::AdaptiveEndPoint(const char* name, std::string ip,
                                   std::string port)
    : Theron::EndPoint(name, ("tcp://" + ip + ":" + port).c_str()) {
  logging_ = new AdaptiveEndPointLogging();

  logging_->log("The AdaptiveEndPoint is created as %s:%s", ip.c_str(),
                port.c_str());
  framework = new Theron::Framework(*(EndPoint*)this);
  connectionActor = new AdaptiveEndPoint::ConnectionActor(
      this, ("ConnectionActor://" + ip + ":" + port).c_str());
...
bool AdaptiveEndPoint::SayHelloToCoordinator(std::string ip, std::string port) {
  ...
  while (connect(socket_coor, (struct sockaddr*)&serv_addr,
                 sizeof(struct sockaddr)) == -1) {
    logging_->elog(
        "Cannot connect to the master %s:%d! To retry in one second!",
        ip_coor.c_str(), atoi(coord_port.c_str()));
    perror("connect to coordinator ");
    sleep(1);
    attemps_budget--;
    if (attemps_budget == 0) {
      logging_->elog("connection errors when connecting to %s:%s! Reason:%s",
  ...
  if (send(socket_coor, &port_send, sizeof(int), 0) == -1) {
    logging_->elog(
        "Error occurs when sending the hello message to the coordinator!\n");
    assert(false);
    return false;
  }
  return true;
...
int AdaptiveEndPoint::GetCoordinatorEndPointPort() {
  ...
  logging_->log(
      "Get CoordinatorEndPoint is successfully! The Coordinator Theron "
      "EndPoint is %s:%d",
      ip_coor.c_str(), port);
  return port;
}
bool AdaptiveEndPoint::WaitForReadySignalFromCoordinator() {
  ...
  if (signal == 'M') {
     ...
     * this node is selected to be loader master,then start a new thread acting
     * master loader

    pthread_t load_master;
    int ret =
        pthread_create(&load_master, NULL,
                       claims::loader::MasterLoader::StartMasterLoader, NULL);
    if (0 == ret) {
      LOG(INFO) << "start master loader......";
    } else {
      PLOG(ERROR) << "can't start master loader!!";
    }
  }*/
  return true;
}
bool AdaptiveEndPoint::ConnectToCoordinateEndPoint(int port) {
  ...
  if (!ConnectToRemoteEndPoint(os.str().c_str())) {
    logging_->elog("Check whether network is enabled! can't connect ot %s",
                   os.str().c_str());
    //    assert(false);
    return false;
  }
...
AdaptiveEndPoint::ConnectionActor::ConnectionActor(AdaptiveEndPoint* AEP,
                                                   const char* Name)
    : Actor(*(AEP->framework), Name), AEP(AEP) {
  RegisterHandler(this,
                  &AdaptiveEndPoint::ConnectionActor::ReceiveNodeStatus256);
}

void AdaptiveEndPoint::ConnectionActor::ReceiveNodeStatus256(
  ...
  AEP->logging_->log(
      "Successfully connected to the EndPoint of new node through %s",
      str.str().c_str());
}
