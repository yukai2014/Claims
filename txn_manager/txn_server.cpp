...
caf::behavior TxnCore::make_behavior() {
  ...
  return {
      ...
      [=](CommitIngestAtom, const Ingest* ingest) -> int {
        if (TxnIndex.find(ingest->Id) == TxnIndex.end()) return -1;
...
caf::behavior TxnWorker::make_behavior() {
  return {[=](IngestAtom, const FixTupleIngestReq& request) -> caf::message {
            Ingest ingest;
          ...
          caf::others >> []() { cout << "work unknown message" << endl; }};
}
...
caf::behavior TxnServer::make_behavior() {
  ...
  return {[=](IngestAtom, const FixTupleIngestReq& request) {
          ...
          [=](CheckpointAtom,
              const UInt64 part) { this->forward_to(caf::spawn<TxnWorker>()); },
