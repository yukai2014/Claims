...
#define MALLOC(size, type) static_cast<type*>(malloc(size * sizeof(type)))
...
//  free_thread_count = thread_count;   //  bug?
    ...
    thread_list_ = MALLOC(thread_count_in_pool, pthread_t);
    while (!task_queue_.empty()) {
        task_queue_.pop();
    }

    for (int i = 0; i < thread_count_; ++i) {
        if (pthread_create(&thread_list_[i], NULL, ThreadExec, this) != 0) {
...
void *ThreadPool::ThreadExec(void *arg) {
    ...
    /*
     * every thread execute a endless loop, waiting for task,
     * and exit when receive a task with end member of 'true'
     */
    while (1) {
        ...
        if (task != NULL) {
            ThreadPoolLogging::log("thread (id=%ld,offset=%lx) "
                                   "in thread pool is executing..\n",
                                   syscall(__NR_gettid), pthread_self());
            task->Run();
            ThreadPoolLogging::log("thread (id=%ld,offset=%lx) "
                                   "in thread pool finished executing..\n",
                                   syscall(__NR_gettid), pthread_self());
            delete task;
            task = NULL;
        }
...
void ThreadPool::BindCpu() {
    ...
    /*
     * 将该子线程的状态设置为detached,
     * 则该线程运行结束后会自动释放所有资源,不要使父线程因为调用pthread_join而阻塞
     */
    ...
    if (ret == -1) {
        ...
        ThreadPoolLogging::log(
                "thread (tid=%ld offset=%lx) bind cpu=%ld, "
                "expect binded cpu=%ld (start=%ld end=%ld)\n",
                syscall(__NR_gettid), pthread_self(), getCurrentCpuAffility(),
                insert_cpu, 0, cpu_count);
    }
}

void ThreadPool::DestroyPool(ThreadPool *tp) {
    // destory every thread by sending destory task to every thread
