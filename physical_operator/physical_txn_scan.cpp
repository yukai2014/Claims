 ...
 *
 * /CLAIMS/physical_operator/physical_txn_scan.cpp
 *
...
#include "../physical_operator/physical_txn_scan.hpp"
...
PhysicalTxnScan::PhysicalTxnScan(State state)
    : state_(state), partition_reader_iterator_(NULL), perf_info_(NULL) {
  InitExpandedStatus();
}

PhysicalTxnScan::PhysicalTxnScan()
...
bool PhysicalTxnScan::Open(const PartitionOffset& kPartitionOffset) {
...
bool PhysicalTxnScan::Next(BlockStreamBase* block) {
...
void PhysicalTxnScan::Print() {
  printf("Scan (ID=%d)\n", state_.projection_id_.table_id);
}

bool PhysicalTxnScan::PassSample() const {
